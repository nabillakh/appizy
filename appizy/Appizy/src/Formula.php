...
class formula {
  var $cell_coord ; // coordonnées de la cellule d'origine
  var $formula_elements ;
  var $dependances ; // Tableau des coordonnées de cellules dont dépent la formule
  var $ext_formula_dependances ; // Liste des fonctions externes dont dépend la formule
  ...
  function formula_convert_cellranges($array_formula = array(),$current_sheet = 0,$sheets_name = array()) {
    ...
    foreach($array_formula as $key => $formula_element) {
      ...
      $temp = str_split($formula_element); // A remplacer par expression régulière
    ...
    // $celldependancies = array_unique($celldependancies); // Chaque dépendance est unique
...
// Décompose une $formula brute (String) en éléments unitaires d'un $lexicon (Array) passé en paramètre
function lexer($formula, $lexicon) {
    ...
    if($size>0)
    {
        // Si le lexique contient un élément on décompose suivante cet élément
        
        // La formule passée en paramètre est décomposée suivant le terme du lexique ODS($lexico)
        list($odslexicon,$javalexicon) = each($lexicon) ;

        $formulapiece = explode($odslexicon,$formula);
        
        // l'array retourné dans ce cas est composé des morceaux de formules que l'on continue de décomposée
        ...
        foreach($formulapiece as $piece)
        {
            ...
            // avant de lancer la récurcivité on vérifie que le morceau de formule n'est pas vide
            if($piece!="") : $lexedformula[$index] = lexer($piece,$newlexicon); endif ;
            $index=$index+2 ;
        }
        // Avant de renvoyer on aplanit le tableau
        $flat_formula = array();
        array_walk_recursive($lexedformula, function($a) use (&$flat_formula) { $flat_formula[] = $a; });

        return $flat_formula;
    }else
    {
        // sinon on renvoie la formule passé en paramètre
        return $formula ;
    }
}
...
// Pour sortir les éléments d'un tableau par taille
function sortBylength($a,$b) {
    return strlen($b)-strlen($a);
}
// Transforme une chaîne alpha en num, base 26
