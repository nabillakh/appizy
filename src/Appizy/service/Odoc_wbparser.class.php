...
class Odoc_wbparser extends Wbparser {
	var $validations;
	var $fonts;
	var $styles;
	var $sheets;
	...
	function order_data() {
		...
		foreach($this->sheets as $cS => $sheet) {
			...
			$this->add_sheet($cS, htmlentities($sheet['TABLE:NAME'],ENT_QUOTES,"UTF-8"));
			...
			foreach($sheet['rows'] as $cR => $row) {
				...
				$this->add_row($cS, $cR, $row_options);
	...
	function startElement($parser, $tagName, $attrs) {
		...
		elseif($cTagName == 'draw:frame') {
			...
			/*
      $this->add_sheet(  $attrs['TABLE:NAME'] );
      */
